...
struct waApp: App {
    ...
    private var isLayoutSmokeTestRequested: Bool {
        ProcessInfo.processInfo.environment["WA_LAYOUT_SMOKETEST"] == "1"
    }
    ...
    var body: some Scene {
        WindowGroup {
            ...
            .onAppear {
                if isLayoutSmokeTestRequested {
                    runLayoutSmokeTestAndTerminate()
                    return
                }
                if !didResetForV2 {
    ...
    private func runLayoutSmokeTestAndTerminate() {
        let passed = runFocusModeLayoutSmokeAssertions()
        let output = passed ? "FOCUS_MODE_LAYOUT_SMOKETEST_OK\n" : "FOCUS_MODE_LAYOUT_SMOKETEST_FAIL\n"
    ...
    private func runFocusModeLayoutSmokeAssertions() -> Bool {
        let sample = String(repeating: "간단하지 않다. ", count: 120)

        func measuredHeight(for width: CGFloat) -> CGFloat {
            let paragraphStyle = NSMutableParagraphStyle()
            paragraphStyle.lineSpacing = 5
            paragraphStyle.lineBreakMode = .byWordWrapping
            let font = NSFont(name: "SansMonoCJKFinalDraft", size: 18)
        ...
        let wider = measuredHeight(for: 900)
        let narrower = measuredHeight(for: 860)
        let hasMonotonicWrap = narrower >= wider
        let hasPositiveHeight = wider > 0 && narrower > 0
        let hasValidMetrics = FocusModeLayoutMetrics.focusModeHorizontalPadding > 0
            && FocusModeLayoutMetrics.focusModeLineFragmentPadding > 0
            && FocusModeLayoutMetrics.focusModeContentPadding > FocusModeLayoutMetrics.focusModeHorizontalPadding

        return hasMonotonicWrap && hasPositiveHeight && hasValidMetrics
    }
