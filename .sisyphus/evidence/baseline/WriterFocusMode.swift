...
extension ScenarioWriterView {
    ...
    func normalizeInactiveFocusModeTextEditorOffsets(
        ...
        sampleObservedHeights: Bool = true
    ) -> (scanned: Int, reset: Int, skippedActive: Int, mapped: Int, observedUpdates: Int) {
        guard showFocusMode else { return (0, 0, 0, 0, 0) }
        guard let root = NSApp.keyWindow?.contentView else { return (0, 0, 0, 0, 0) }
        ...
        var observedBodyByCard: [UUID: CGFloat] = [:]
        ...
        for textView in collectTextViews(in: root) {
            ...
            if sampleObservedHeights {
                ...
                if let cardID = mappedCardID, focusedCardIDs.contains(cardID) {
                    ...
                    if let layoutManager = textView.layoutManager,
                       let textContainer = textView.textContainer {
                        ...
                        if usedHeight > 0 {
                            let observedBody = max(1, ceil(usedHeight + focusModeBodySafetyInset))
                            if abs((observedBodyByCard[cardID] ?? 0) - observedBody) > 0.5 {
                                observedBodyByCard[cardID] = observedBody
                                observedUpdates += 1
                            }
                        }
                    }
        ...
        if focusObservedBodyHeightByCardID != observedBodyByCard {
            focusObservedBodyHeightByCardID = observedBodyByCard
        }
        return (scanned, resetCount, skippedActive, mapped, observedUpdates)
    }

    func requestFocusModeOffsetNormalization(
        includeActive: Bool = false,
        force: Bool = false,
        allowObservedSampling: Bool = true,
        ...
        _ = normalizeInactiveFocusModeTextEditorOffsets(
            includeActive: includeActive,
            sampleObservedHeights: allowObservedSampling
        )
    }
    ...
    func requestFocusModeCaretEnsure(typewriter: Bool, delay: Double = 0.016, force: Bool = false, reason: String = "unspecified") {
        ...
        let work = DispatchWorkItem {
            ...
            requestFocusModeOffsetNormalization(allowObservedSampling: false, reason: "caret-ensure")
        }
