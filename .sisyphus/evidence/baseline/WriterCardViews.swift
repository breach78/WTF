...
private enum FocusModeTextHeightCalculator {
    ...
    static func measureBodyHeight(
        ...
        // Trailing newline creates a visual empty line in NSTextView.
        // Append a sentinel space so measurement includes that final line immediately.
...
struct FocusModeCardEditor: View {
    ...
    @State private var activeMeasuredHeight: CGFloat = 0
    @State private var inactiveMeasuredHeight: CGFloat = 0
    @State private var editorBodyHeight: CGFloat = 0
    ...
    private var targetMeasuredHeight: CGFloat {
        let primary = isActive ? activeMeasuredHeight : inactiveMeasuredHeight
        if primary > 1 { return primary }
        return max(activeMeasuredHeight, inactiveMeasuredHeight)
    }
    private var textEditorBodyHeight: CGFloat {
        max(1, targetMeasuredHeight - (verticalInset * 2))
    }
    ...
    private var activeSizingText: String {
        let text = card.content
        if text.isEmpty { return " " }
        return text
    }
    private var inactiveSizingText: String {
        return activeSizingText
    }

    private func liveResponderBodyHeight() -> CGFloat? {
    ...
    private func refreshMeasuredHeights(
        reason: String,
        alwaysEmit: Bool = false,
        skipInactiveMeasureWhenActive: Bool = false
    ) {
        guard measuredCardWidth > 1 else {
            return
        }
        let liveBody = liveResponderBodyHeight()
        let activeBody: CGFloat = {
            if let liveBody {
                return liveBody
            }
            return FocusModeTextHeightCalculator.measureBodyHeight(
                text: activeSizingText,
                fontSize: focusModeFontSize,
                lineSpacing: focusModeLineSpacing,
                width: textEditorMeasureWidth
            )
        }()
        let inactiveBody: CGFloat = {
            let shouldSkipInactiveMeasure = isActive && skipInactiveMeasureWhenActive
            if shouldSkipInactiveMeasure {
                let cached = max(1, inactiveMeasuredHeight - (verticalInset * 2))
                return cached
            }
            return FocusModeTextHeightCalculator.measureBodyHeight(
                text: inactiveSizingText,
                fontSize: focusModeFontSize,
                lineSpacing: focusModeLineSpacing,
                width: textEditorMeasureWidth
            )
        }()
        let observedBody = observedBodyHeight ?? 0
        // Runtime observation can transiently under-report during relayout.
        // Never let it shrink below deterministic text-layout measurement.
        let resolvedActiveBody = observedBody > 1 ? max(activeBody, observedBody) : activeBody
        let resolvedInactiveBody = inactiveBody
        let newActiveTotal = resolvedActiveBody + (verticalInset * 2)
        let newInactiveTotal = resolvedInactiveBody + (verticalInset * 2)
        let newEditorBody = isActive ? resolvedActiveBody : resolvedInactiveBody
        let newRoot = isActive ? newActiveTotal : newInactiveTotal
        if abs(activeMeasuredHeight - newActiveTotal) > 0.25 {
            activeMeasuredHeight = newActiveTotal
        }
        if abs(inactiveMeasuredHeight - newInactiveTotal) > 0.25 {
            inactiveMeasuredHeight = newInactiveTotal
        }
        if abs(editorBodyHeight - newEditorBody) > 0.25 {
            editorBodyHeight = newEditorBody
        }
        if abs(rootHeight - newRoot) > 0.25 {
            rootHeight = newRoot
        }
    }
    ...
    var body: some View {
        ...
        .onPreferenceChange(FocusModeCardWidthPreferenceKey.self) { value in
            ...
            refreshMeasuredHeights(reason: "pref-width")
        }
        .onAppear {
            refreshMeasuredHeights(reason: "appear", alwaysEmit: true)
        }
        .onChange(of: card.content) { oldValue, newValue in
            ...
            refreshMeasuredHeights(
                ...
                alwaysEmit: true,
                skipInactiveMeasureWhenActive: true
            )
        }
        .onChange(of: isActive) { _, _ in
            refreshMeasuredHeights(reason: "active-change", alwaysEmit: true)
        }
        .onChange(of: fontSize) { _, _ in
            refreshMeasuredHeights(reason: "font-change")
        }
        .onChange(of: focusModeLineSpacingValue) { _, _ in
            refreshMeasuredHeights(reason: "line-spacing-change")
        }
        .onChange(of: observedBodyHeight) { _, _ in
            refreshMeasuredHeights(reason: "observed-body-change", alwaysEmit: true)
        }
        .contentShape(Rectangle())
